...
def to_rexpr(root):             # noqa C901
    ...
    if root.type is Operator("scale"):
        return "rescale(%s, newrange = c(%d, %d))" % (
            to_rexpr(root.args[0]),
            root.args[1],
            root.args[2],
        )
...
def to_repr(root):              # noqa C901
    ...
    if root.type is Operator("scale"):
        ...
        raise ValueError(
            "unexpected number of arguments for scale: %s"
            % ", ".join(map(str, root.args))
        )
    if root.type is Operator("pow"):
        return "(pow(%s, %s))" % (to_repr(root.args[0]), to_repr(root.args[1]))
    if root.type is Operator("max"):
        return "(max(%s, %s))" % (to_repr(root.args[0]), to_repr(root.args[1]))
    if root.type is Operator("min"):
        return "(min(%s, %s))" % (to_repr(root.args[0]), to_repr(root.args[1]))
...
def to_expr(root, ctx=None):                            # noqa C901
    ...
    if root.type is Operator("sel"):
        ...
        if guvec():
            ...
            if isinstance(root.args[0], Node) and root.args[0].type is Operator("in"):
    ...
    if root.type is Operator("poly"):
        assert len(root.args) == 4, "norm2 and alpha not set"
        return "(poly.ortho_poly_predict(%s, %s, %s, %d))" % (
            recurse(root.args[0]),
            recurse(root.args[2]),
            recurse(root.args[3]),
            root.args[1],
    ...
    if root.type is Operator("pow"):
        return "(np.power(%s, %s))" % (recurse(root.args[0]), recurse(root.args[1]))
    if root.type is Operator("max"):
        return "(np.maximum(%s, %s))" % (recurse(root.args[0]), recurse(root.args[1]))
    if root.type is Operator("min"):
        return "(np.minimum(%s, %s))" % (recurse(root.args[0]), recurse(root.args[1]))
    if root.type is Operator("clip"):
        return "(np.clip(%s, %s, %s))" % (
            recurse(root.args[0]),
            recurse(root.args[1]),
            recurse(root.args[2]),
        )
...
def find_input_types(root):
    ...
    for node in root.walk():
        if (
            ...
            and node.type is Operator("in")
            and node.args[0] not in types
        ):
...
def to_pyx(root, fname):
    lsyms = find_syms(root)
    decls = ["cdef np.ndarray %s = %s" % (name, lsyms[name]) for name in lsyms.keys()]
    inputs = find_inputs(root)
    prelude = """
cimport cython
import r2py.poly as poly
...
"""
    expr = "return " + to_expr(root)
    iodecls = [
        "np.ndarray[DTYPE_t, ndim=1] %s not None" % v for v in sorted(set(inputs))
    ]
...
"""
    ...
    if nonvector:
        ...
        stmts = [
            "var_%d = %s" % (name, to_expr(nonvector[name]))
...
""".format(
        prelude=impts if not child else "",
        body=body,
        fname=fname,
        params=", ".join(params),
